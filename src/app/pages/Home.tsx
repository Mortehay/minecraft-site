...
import { Copy, Check, Youtube, Calendar, ArrowRight, Loader2 } from 'lucide-react';
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2">
            <div className="flex items-center justify-between mb-6">
              ...
              <Link to="/news" className="text-sm text-[#10b981] hover:underline flex items-center gap-1">
              ...
              {newsLoading ? (
                ...
                news.map(item => (
                  <Link key={item.id} to={`/news/${item.id}`} className="block group">
                    <GlassCard className="group-hover:border-[#10b981]/50 transition-colors">
                      <div className="flex gap-6">
                        {item.image_url ? (
                          ...
                          <div className="w-32 h-32 bg-[#1a1a2e] rounded flex items-center justify-center flex-shrink-0">
                            <span className="text-2xl">📰</span>
                          </div>
                        )}
                        <div className="flex-1">
                          <h3 className="text-xl mb-2 text-[#e8e8ea] group-hover:text-[#10b981] transition-colors">{item.title}</h3>
                          <div className="flex items-center gap-2 text-xs text-[#9ca3af] mb-3">
                            <Calendar className="w-3 h-3" />
                            {new Date(item.created_at).toLocaleDateString()}
                          </div>
                          <p className="text-[#9ca3af] line-clamp-2 text-sm">{item.content?.body || ''}</p>
                        </div>
                      </div>
                    </GlassCard>
                  </Link>
                ...
                <p className="text-[#9ca3af] italic py-8 text-center bg-[#13131a] rounded border border-dashed border-white/5">No news posts yet.</p>
              )}
            </div>
          </div>
            ...
            <div>
              ...
              <GlassCard>
                <div className="text-center">
                  ...
                  <div className="w-48 h-48 mx-auto bg-white p-2 rounded">
                    {/* QR Code Placeholder */}
                    <div className="w-full h-full bg-black/90 flex flex-col items-center justify-center border-2 border-black/10">
                      <div className="text-white text-[10px] font-bold">EPIC MC MOBILE</div>
                      <div className="w-24 h-24 border-2 border-dashed border-white/20 mt-2" />
                    </div>
                  </div>
